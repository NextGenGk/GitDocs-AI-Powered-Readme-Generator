...
import { FileText, AlertCircle, CheckCircle, Github, Download, Copy, Edit, Eye, RefreshCw, X } from 'lucide-react';

const formSchema = z.object({
    githubUrl: z.string().url().refine(u => /^https?:\/\/github\.com/.test(u)),
});
...
export default function Dashboard() {
                ...
                if (currentLineIndex < lines.length) {
                    ...
                    if (currentCharIndex < currentLine.length) {
                        ...
                        setDisplayedMarkdown(prev => {
                            const newLines = [...lines.slice(0, currentLineIndex), currentLine.slice(0, currentCharIndex + 1)];
                            return newLines.join('\n');
                        });
                        currentCharIndex++;
                        typewriterRef.current = setTimeout(typeNextChar, 5); // Adjust speed here
                    } else {
                        // Move to next line
                        currentLineIndex++;
                        currentCharIndex = 0;
                        setDisplayedMarkdown(prev => prev + '\n');
                        typewriterRef.current = setTimeout(typeNextChar, 20); // Pause between lines
                    }
                } else {
                    // Typing complete
                    setIsTyping(false);
                    setDisplayedMarkdown(markdown);
                    setEditedContent(markdown); // Set edited content when typing is complete
                }
            };

            typewriterRef.current = setTimeout(typeNextChar, 300); // Initial delay
    ...
    const copyToClipboard = async () => {
        try {
            const contentToCopy = isEditing ? editedContent : displayedMarkdown;
    ...
    const downloadMarkdown = () => {
        const contentToDownload = isEditing ? editedContent : displayedMarkdown;
    ...
    const handleEditToggle = () => {
        if (isEditing) {
            // Save changes and trigger typewriter effect
            setMarkdown(editedContent);
            setIsEditing(false);
        } else {
            // Enter edit mode
            setEditedContent(displayedMarkdown);
            setIsEditing(true);
        }
    };
        ...
        <div className="min-h-screen py-22 bg-white relative overflow-hidden">
            ...
            <div className="container mx-auto px-4 py-8 md:py-16">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8 max-w-7xl mx-auto">
                    {/* Input Section */}
                    <div className="w-full shadow-lg border rounded-lg hover:border-gray-300 transition-all duration-300 overflow-hidden">
                        <div className="absolute top-0 left-0 w-32 h-32 bg-blue-50 rounded-full -translate-y-1/2 -translate-x-1/2 blur-xl z-0"></div>
                        <div className="relative z-10 bg-gray-50 border-b p-4">
                        ...
                        <div className="space-y-6 p-6 relative z-10">
                            {error && (
                                <div className="bg-red-50 border rounded-md p-3">
                                        ...
                                        render={({ field, fieldState }) => (
                                            <div className="space-y-1">
                                                <Input
                                                    ...
                                                    className="w-full px-3 py-2 border rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-gray-50"
                                ...
                                {markdown && (
                                    <div className="flex gap-2 flex-wrap">
                                        <Button
                                            ...
                                            className="flex items-center gap-1 px-3 py-1 border rounded-md text-sm text-blue-600 hover:bg-blue-50 transition-colors"
                                            disabled={isTyping}
                                        >
                            ...
                            <div className="h-64 md:h-80 lg:h-96 border rounded-md overflow-hidden bg-gray-50 shadow-inner">
                                {markdown ? (
                                    isEditing ? (
                                        ...
                                        <div className="w-full h-full p-4 overflow-auto bg-white relative">
                                            <div
                                                className="prose prose-sm max-w-none text-gray-800"
                                                dangerouslySetInnerHTML={{
                                                    __html: renderMarkdownPreview(displayedMarkdown)
                                                }}
                                            />
                                        </div>
                                    )
                                    ...
                                    <div className="flex items-center justify-center h-full text-gray-500">
                                        <div className="text-center space-y-3">
                                            ...
                                            <div className="inline-block bg-blue-50 text-blue-600 border px-3 py-1 rounded-full text-sm">
                                                Markdown Preview
                                            </div>
                                        </div>
                                    </div>
                ...
                {/* Usage Tracking Card */}
                <div className="mt-8 mb-6 md:mt-12 md:mb-8 max-w-7xl mx-auto shadow-lg border rounded-lg overflow-hidden">
                    ...
                    <div className="space-y-4 p-6 relative z-10">
                        ...
                        {usageInfo.isLimitReached && (
                            <>
                                ...
                                <div className="border bg-red-50 rounded-md p-4">
                      ...
                      <span className="text-red-700 text-sm">
                        Generation limit reached! You have used all {usageInfo.maxGenerations} available generations.
                                            ...
                                            {/*  variant="outline"*/}
                                            {/*  size="sm"*/}
                                            {/*  className="ml-4 px-3 py-1 border rounded-md text-sm text-red-600 hover:bg-red-100 transition-colors"*/}
                    ...
                    <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] flex flex-col">
                        ...
                        <div className="flex-1 overflow-auto p-6">
                            <div
                                className="prose prose-sm max-w-none text-gray-800"
                                dangerouslySetInnerHTML={{
                                    __html: renderMarkdownPreview(isEditing ? editedContent : displayedMarkdown)
                                }}
                            />
                        </div>
                    </div>
